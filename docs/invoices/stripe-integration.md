...
**Next Steps:** Learn how to [connect your Stripe account](#connecting-stripe-to-shootpath) or understand [how payments are processed](#how-stripe-processes-payments).
...
**Quick estimate:** ~3% of each payment goes to Stripe.
...
Stripe supports international clients, but there are some considerations:

#### Accepting International Cards

**Most international cards work:**
- Visa/Mastercard issued anywhere
- Amex from most countries
- Debit cards from major banks

**May not work:**
- Some local card networks (China UnionPay in some cases)
- Prepaid cards from certain countries
- Cards with international transaction blocks

**Client experience:**
- Enter card number same as domestic
- May need to call bank to authorize international purchase
...
- Their bank converts from their local currency

**Exchange rate:**
- Client's bank sets exchange rate
- You always receive USD

**Example:**
- You invoice $1,000
- UK client pays with GBP card
- Their bank charges ~£800 (current exchange rate)
- You receive $1,000 USD

**No currency risk for you!** The client's bank handles conversion.

### Stripe Alternatives

ShootPath is built specifically for Stripe. Here's why:

**Why Stripe (not PayPal, Square, etc.):**
- Developer-friendly API (easy for ShootPath to integrate)
- Best fraud detection in the industry
- Transparent pricing (no hidden fees)
- Excellent documentation and support
- Industry standard for SaaS platforms

**Can I use a different processor?**
No, ShootPath only integrates with Stripe currently. This keeps the integration simple, secure, and reliable.

**If you already use Square/PayPal for in-person payments:**
You can still use them for those! Stripe is just for ShootPath online invoices. Many photographers use:
- Stripe for online invoices (via ShootPath)
- Square for in-person card readers at events
- Venmo/Zelle for informal payments

**They can coexist!** Just track all revenue sources for taxes.
...
**Need to track who's paid?** → [Tracking Payments](./tracking-payments) shows how to monitor invoice status.

**Want to understand invoicing basics?** → [Invoices Overview](./index) covers the big picture.

**Ready to set up payment schedules?** → [Payment Schedules](./payment-schedules) explains automatic invoice generation.

**Need to create a custom invoice?** → [Creating Invoices](./creating-invoices) walks through manual invoices.
